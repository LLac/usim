// STEPNotesTab.cpp : implementation file
//
...
CSTEPNotesTab::CSTEPNotesTab(CWnd* pParent /*=NULL*/)
	: CResizableDialog(CSTEPNotesTab::IDD, pParent)
{
	//{{AFX_DATA_INIT(CSTEPNotesTab)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}

void CSTEPNotesTab::DoDataExchange(CDataExchange* pDX)
{
	CResizableDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CSTEPNotesTab)
...
BEGIN_MESSAGE_MAP(CSTEPNotesTab, CResizableDialog)
	//{{AFX_MSG_MAP(CSTEPNotesTab)
...
/////////////////////////////////////////////////////////////////////////////
// CSTEPNotesTab message handlers

BOOL CSTEPNotesTab::OnInitDialog() 
{
...
void CSTEPNotesTab::InitNotesTab() 
{
	if (theApp.m_CurrenthItem == NULL || theApp.m_pDevArray[theApp.m_CurDevIndex] == NULL) {
		...
		m_Desc.SetWindowText(theApp.m_pDevArray[theApp.m_CurDevIndex]->m_STEPNotesArray[theApp.m_CurItemIndex].m_Desc);
		SetTreeItemDesc();
		m_Notes.SetWindowText(theApp.m_pDevArray[theApp.m_CurDevIndex]->m_STEPNotesArray[theApp.m_CurItemIndex].m_Notes);
	}
}

void CSTEPNotesTab::SetTreeItemDesc()
{
...
void CSTEPNotesTab::OnChangeDesc()
{
	if (theApp.m_CurrenthItem == NULL) return;

	m_Desc.GetWindowText(theApp.m_pDevArray[theApp.m_CurDevIndex]->m_STEPNotesArray[theApp.m_CurItemIndex].m_Desc);
	SetTreeItemDesc();
}

void CSTEPNotesTab::OnChangeNotes()
{
	if (theApp.m_CurrenthItem == NULL) return;

	m_Notes.GetWindowText(theApp.m_pDevArray[theApp.m_CurDevIndex]->m_STEPNotesArray[theApp.m_CurItemIndex].m_Notes);
}

void CSTEPNotesTab::OnClear() 
{
	m_Desc.SetWindowText("");
	m_Notes.SetWindowText("");
}

void CSTEPNotesTab::OnClearButton() 
{
	OnClear();
	theApp.m_pDevArray[theApp.m_CurDevIndex]->m_STEPNotesArray[theApp.m_CurItemIndex].Clear();

	CString str;
	str.Format("%s : ",	theApp.m_pTreeDevices->GetItemLabel(theApp.m_pDevArray[theApp.m_CurDevIndex]->m_STEPhItem[theApp.m_CurItemIndex]));
	theApp.m_pTreeDevices->SetItemText(theApp.m_pDevArray[theApp.m_CurDevIndex]->m_STEPhItem[theApp.m_CurItemIndex], str);
}

BOOL CSTEPNotesTab::OnCommand(WPARAM wParam, LPARAM lParam) 
{
