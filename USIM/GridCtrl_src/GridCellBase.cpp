        ...
        if (pGrid->GetImageList()->GetImageInfo(GetImage(), &Info))
        {
            ...
            */
			...
			if (GetFormat() & DT_RIGHT) 
				trect.left = rect.right - nImageWidth;
			if (GetFormat() & DT_CENTER)
				trect.left += ((rect.right - rect.left)/2 - nImageWidth/2);
            if ((nImageWidth + trect.left <= rect.right) && (nImageHeight + rect.top <= rect.bottom))                
                pGrid->GetImageList()->Draw(pDC, GetImage(), trect.TopLeft(), ILD_NORMAL);
