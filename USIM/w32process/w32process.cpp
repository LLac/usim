...
PROCESS_INFORMATION_EX*	CProcessList::FindModule(const LPTSTR pstrExe)
{
	CString str;

	for(int i = 0; i < GetSize(); ++i)
	{
		str = _strupr(GetAt(i).szModule);
		if(strcmp(str, _strupr(pstrExe)) == NULL && !str.IsEmpty())
		{
			return &(GetAt(i));
		}
	}
