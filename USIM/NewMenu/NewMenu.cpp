  ...
  if (nItemIndex!=UINT(-1) && GetMenuItemRect(pWnd->m_hWnd,pMenu->m_hMenu, nItemIndex, &itemRect))
  {
    ...
    if(pMenu->GetMenuItemInfo(nItemIndex,&menuInfo,TRUE))
    {
      ...
      if(menuInfo.fType&MF_OWNERDRAW)
      {
        ...
        CFont fontMenu;
		LOGFONT logFontMenu = CNewMenu::GetNeededFontTN();
//         LOGFONT logFontMenu;
// 
// #ifdef _NEW_MENU_USER_FONT
...
void CNewMenu::DrawItem_WinXP(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
  ...
  else
  {
    ...
    if(!strText.IsEmpty())
    {
		LOGFONT logFontMenu = GetNeededFontTN();
//      LOGFONT logFontMenu;
      CFont fontMenu;
...
// 		  nm.cbSize -= sizeof(nm.iPaddedBorderWidth);
// 	  
// #endif
// 
//       VERIFY (SystemParametersInfo(SPI_GETNONCLIENTMETRICS,nm.cbSize,&nm,0));
...
//********************* Added by Tiushkov Nikolay
int CNewMenu::GetNONCLIENTMETRICSSize()
{
#if (WINVER < 0x0600)
...
*/
	...
	if (OSVersionInfo.dwMajorVersion < 6) 
		return sizeof(NONCLIENTMETRICS) - sizeof(int); // nm.iPaddedBorderWidth int
	else
		return sizeof(NONCLIENTMETRICS);
#endif
}

LOGFONT CNewMenu::GetNeededFontTN()
{

#ifdef _NEW_MENU_USER_FONT
	return MENU_USER_FONT;
#else
	
	NONCLIENTMETRICS nm = {0};
	static const int sizeNONCLIENTMETRICS = GetNONCLIENTMETRICSSize();
	nm.cbSize = sizeNONCLIENTMETRICS;

	VERIFY (SystemParametersInfo(SPI_GETNONCLIENTMETRICS,nm.cbSize,&nm,0));
	return nm.lfMenuFont; 
	// TODO : Probably we can cache it?
#endif
}
