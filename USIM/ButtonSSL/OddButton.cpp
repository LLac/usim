...
#define BSS_TYPEMASK SS_TYPEMASK
...
void COddButton::PreSubclassWindow() 
{
	...
	m_nTypeStyle = GetButtonStyle() & BSS_TYPEMASK;
	...
	ModifyStyle(BSS_TYPEMASK, BS_OWNERDRAW, SWP_FRAMECHANGED);
...
{
	UINT nNewType = (wParam & BSS_TYPEMASK);
	...
	return DefWindowProc(BM_SETSTYLE,
		(wParam & ~BSS_TYPEMASK) | BS_OWNERDRAW, lParam);
}
...
void COddButton::EnableDefault(BOOL bEnable)
{
	...
	if (!bEnable && m_bDefault)
	{
		...
		SendMessage(BM_SETSTYLE, 
			(GetStyle() & ~BSS_TYPEMASK) | BS_PUSHBUTTON, TRUE);
