// STEPTabCtrl.cpp: implementation of the CSTEPTabCtrl class.
//
//////////////////////////////////////////////////////////////////////
...
BEGIN_MESSAGE_MAP(CSTEPTabCtrl, CXTabCtrl)
	//{{AFX_MSG_MAP(CSTEPTabCtrl)
	ON_WM_SIZE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

void CSTEPTabCtrl::InitTabs()
{
	...
	m_STEP = m_ImageList.Add(theApp.LoadIcon(IDI_STEPS));
	m_STEP_P = m_ImageList.Add(theApp.LoadIcon(IDI_STEPS_P));
	...
	m_STEPTab.Create (IDD_SPIDAC, this);
	AddTab (&m_STEPTab, _T("Stepper Output"), m_STEP);
	m_STEPTab.m_pTab = this;

	m_Init = true;
}

void CSTEPTabCtrl::InitAllTabs()
{
	m_NotesTab.InitNotesTab();
	m_FlagsTab.InitFlagsTab();
	m_STEPTab.InitSTEPTab();
}

void CSTEPTabCtrl::OnSize(UINT nType, int cx, int cy) 
{
	...
	m_STEPTab.GetWindowRect(&lpRect);
	ScreenToClient(&lpRect);
	lpRect.bottom = cy - 10;
	lpRect.right = cx - 10;
	m_STEPTab.MoveWindow(&lpRect, TRUE);
}