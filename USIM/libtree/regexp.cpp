...
TCHAR* CRegExp::GetReplaceString( const TCHAR* sReplaceExp )
{
	...
	while ((c = *src++) != _T('\0')) 
	{
					...
					endp[no] > startp[no]) 
		{
			...
			// k.f. added start //
			m_pVars[no] = sFoundText  + tagpos;
			m_VarLen[no] = len ;
			m_iVarCount++ ;					// ?? or max(m_iVarCount, no) ?? //
			// k.f. added end //
...
#ifdef STRINGEX
	// Here's a function that will do global search and replace using regular  //
	// expressions. Note that the CStringEx class described in the earlier     //
	// section is being used here. The main reason for using CStringEx is that // 
	// it provides the Replace() function which makes our task easier.		   //
...
// k.f. added //
int CRegExp::GetVar(int iVarNr, CString &RetString)
{
	if (iVarNr < 1)
		return -2 ;
	...
	RetString = CString(m_pVars[iVarNr]) ;
	RetString = RetString.Left(m_VarLen[iVarNr]) ;

	return 1 ;
}

// k.f. added //
int CRegExp::GetVarCount()
{
	return m_iVarCount ;
}
