...
void COptionTreeItem::SetAttributeText(CString strAttribute)
{
	// Set Attribute
	m_strAttribute = strAttribute;
}

CString COptionTreeItem::GetAttributeText()
{
	// Return Attribute
...
UINT COptionTreeItem::GetListType()
{
	return m_ListType;
}

void COptionTreeItem::SetListType(UINT iType)
{
	// Set variables
	m_ListType = iType;
}
...
void COptionTreeItem::DrawButtons(CDC *pDC, const CRect &rcRect,const int code,const bool bhighlight)
{
  ...
  {
	tmpRect = GetButtonRect(m_rcButtons,_D_BT_PLUS);
    this->DrawButton("+",pDC,GetBrushBTAdd(bhighlight),tmpRect,bhighlight);
  }
  if(_btqueue & _D_BT_MINUS && (code == _D_BT_ALL || code == _D_BT_MINUS))
  {
	tmpRect = GetButtonRect(m_rcButtons,_D_BT_MINUS);
    this->DrawButton("-",pDC,GetBrushBTDelete(bhighlight),tmpRect,bhighlight);
  }
  if(_btqueue & _D_BT_DEFAULT && (code == _D_BT_ALL || code == _D_BT_DEFAULT))
  {
	tmpRect = GetButtonRect(m_rcButtons,_D_BT_DEFAULT);
    this->DrawButton("D",pDC,GetBrushBTDefault(bhighlight),tmpRect,bhighlight);
  }
  if(_btqueue & _D_BT_USER && (code == _D_BT_ALL || code == _D_BT_USER))
  {
	tmpRect = GetButtonRect(m_rcButtons,_D_BT_USER);
    this->DrawButton(_text,pDC,GetBrushBTBTUser(bhighlight),tmpRect,bhighlight);
  }
  pDC->SetTextColor(crOld);
  pDC->SetBkMode(nOldBack);
  pDC->SelectObject(pOldFont);
