	...
	if (theApp.m_CurrenthItem == NULL || theApp.m_pDevArray[theApp.m_CurDevIndex] == NULL) {
		...
		switch (theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_SimType) {
			case SIM_XPLANE:
				...
				// blink section
				if (XPDataArray[theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarBlinkTokenIndex].VarArraySize > 0) {
					str2.Format("X-Plane/%s[%d]", theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarBlinkTokenName,
						theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarArrayBlinkTokenIndex + 1);
				}
				else {
					str2.Format("X-Plane/%s", theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarBlinkTokenName);
				}
				break;
			...
			case SIM_IL2PF:
				...
				str2 = theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarBlinkTokenName;
				break;
		...
		switch (theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BitSimType) {
			case LFSBITS:
		...
		for (i=0; i<mask_end; i++) {
			if (theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BitMask == pBitsArray[i].BitMask &&
				theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BitSimType == pBitsArray[i].BitSimType) {
				m_Mask.SetWindowText(pBitsArray[i].TokenString);
			}
		}

		m_BlinkMask.SetValue(theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkMask, true);

		switch (theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkSimType) {
			case LFSBITS:
				mask_end = ArraySize(LFSBitsArray);
				pBitsArray = LFSBitsArray;
				break;
		...
		for (i = 0; i < mask_end; i++) {
			if (theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkMask == pBitsArray[i].BitMask &&
				theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkSimType == pBitsArray[i].BitSimType) {
				m_Blink.SetWindowText(pBitsArray[i].TokenString);
			}
		}

		m_BlinkOnTime.SetInt(theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkOnTime);
		m_BlinkOffTime.SetInt(theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkOffTime);

		if (!theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarTokenName.IsEmpty()) {
			m_OVGroup1.SetEnable(true);
			m_OVGroup2.SetEnable(true);
			m_OBGroup3.SetEnable(true);
			m_OBGroup4.SetEnable(true);
			if (theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_Condition2 >> 0)
				m_OBGroup2.SetEnable(true);
			OnSelchangeOpCombo();

			if (theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarBlinkTokenName.IsEmpty())
...
void COutputTab::OnChangeBlinkMask()
{
	if (theApp.m_CurrenthItem == NULL) return;

	theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkMask = m_BlinkMask.GetValue();
}

void COutputTab::OnChangeOnTime()
{
	if (theApp.m_CurrenthItem == NULL) return;

	theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkOnTime = (UINT)m_BlinkOnTime.GetInt();
}

void COutputTab::OnChangeOffTime()
{
	if (theApp.m_CurrenthItem == NULL) return;

	theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkOffTime = (UINT)m_BlinkOffTime.GetInt();
}
...
void COutputTab::OnFlightBlinkData()
{
	SetCursor(AfxGetApp()->LoadStandardCursor(IDC_WAIT));
	m_VarListDlg.m_ListTypeVar = VARTYPE_VAR_ONLY;

	m_VarListDlg.m_VarTokenName = theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarBlinkTokenName;
	m_VarListDlg.m_VarArrayIndex = theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarArrayBlinkTokenIndex;
	int nResponse = m_VarListDlg.DoModal();

	if (nResponse == IDOK) {
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarBlinkTokenName = m_VarListDlg.m_VarTokenName;
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarBlinkTokenIndex = m_VarListDlg.m_ArrayIndex;
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarArrayBlinkTokenIndex = m_VarListDlg.m_VarArrayIndex;
...
void COutputTab::OnClearMask() 
{
	...
	theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BitSimType = 0;
	m_Mask.SetWindowText("");
	m_HexMask.SetValue(0, true);
}

void COutputTab::OnClearBlinkData()
{
	theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarBlinkTokenName.Empty();
	theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarBlinkTokenIndex = 0;
	theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarArrayBlinkTokenIndex = 0;
	m_BlinkData.SetWindowText("");
	OnClearBlinkMask();
	m_OBGroup4.SetEnable(false);
}

void COutputTab::OnClearBlinkMask()
{
	theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkMaskName.Empty();
	theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkMask = 0;
	theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkSimType = 0;
	m_Blink.SetWindowText("");
	m_BlinkMask.SetValue(0, true);
	m_BlinkOnTime.SetInt(0);
	m_BlinkOffTime.SetInt(0);
...
void COutputTab::OnMaskdata() 
{
	...
	m_VarListDlg.m_VarTokenName = theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BitMaskName;
	...
	if (nResponse == IDOK) {
		...
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BitSimType = m_VarListDlg.m_BitSimType;
...
void COutputTab::OnBlinkMaskData()
{
	SetCursor(AfxGetApp()->LoadStandardCursor(IDC_WAIT));
	m_VarListDlg.m_ListTypeVar = VARTYPE_BITS;

	m_VarListDlg.m_VarTokenName = theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkMaskName;
	//	m_VarListDlg.m_VarArrayIndex = theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_VarArrayBlinkTokenIndex;

	int nResponse = m_VarListDlg.DoModal();

	if (nResponse == IDOK) {
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkMaskName = m_VarListDlg.m_VarTokenName;
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkMask = m_VarListDlg.m_BitMask;
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_OutputArray[theApp.m_CurItemIndex].m_BlinkSimType = m_VarListDlg.m_BitSimType;
