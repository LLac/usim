// OutputTabCtrl.cpp: implementation of the COutputTabCtrl class.
//
//////////////////////////////////////////////////////////////////////
...
BEGIN_MESSAGE_MAP(COutputTabCtrl, CXTabCtrl)
	//{{AFX_MSG_MAP(COutputTabCtrl)
	ON_WM_SIZE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

void COutputTabCtrl::InitTabs()
{
	...
	m_OUTPUT = m_ImageList.Add(theApp.LoadIcon(IDI_OUTPUT));
	m_OUTPUT_P = m_ImageList.Add(theApp.LoadIcon(IDI_OUTPUT_P));
	...
	m_OutputTab.Create (IDD_OUTPUTS, this);	
	AddTab (&m_OutputTab, _T("Direct Output"), m_OUTPUT);
	m_OutputTab.m_pTab =this;

	m_Init = true;
}

void COutputTabCtrl::InitAllTabs()
{
	m_NotesTab.InitNotesTab();
	m_FlagsTab.InitFlagsTab();
	m_OutputTab.InitOutputTab();
}

void COutputTabCtrl::OnSize(UINT nType, int cx, int cy) 
{
	...
	m_OutputTab.GetWindowRect(&lpRect);
	ScreenToClient(&lpRect);
	lpRect.bottom = cy - 10;
	lpRect.right = cx - 10;
	m_OutputTab.MoveWindow(&lpRect, TRUE);
}