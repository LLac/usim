// DotMXTab.cpp : implementation file
//
...
CDotMXTab::CDotMXTab(CWnd* pParent /*=NULL*/)
	: CResizableDialog(CDotMXTab::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDotMXTab)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}

void CDotMXTab::DoDataExchange(CDataExchange* pDX)
{
	CResizableDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDotMXTab)
...
BEGIN_MESSAGE_MAP(CDotMXTab, CResizableDialog)
	//{{AFX_MSG_MAP(CDotMXTab)
...
/////////////////////////////////////////////////////////////////////////////
// CDotMXTab message handlers

BOOL CDotMXTab::OnInitDialog() 
{
	...
	m_Grid.SetColumnCount(DMX_NUM_COLS);
	...
	if (theApp.m_CurrenthItem == NULL || theApp.m_pDevArray[theApp.m_CurDevIndex] == NULL) {
		...
		for (j = 0; j < 4; j++) {
			if (theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[j].Grouping != 0 ||
				...
				switch(theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[j].SimType) {
					case SIM_XPLANE:
						if (XPDataArray[theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[j].VarTokenIndex].VarArraySize > 0) {
							str.Format("X-Plane/%s[%d]", theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[j].VarTokenName,
								theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[j].VarArrayTokenIndex + 1);
						} else {
							str.Format("X-Plane/%s", theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[j].VarTokenName);
						}
						break;
				...
				m_Grid.SetItemData (m_CurrentRow, 2, theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[j].SimType);

				m_Grid.SetItemText(m_CurrentRow, 2, theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[j].UserValue);
				str.Format(_T("%.7f"), theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[j].Factor);
				m_Grid.SetItemText(m_CurrentRow, 3, str);
				str.Format(_T("%.7f"), theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[j].Constant);
				m_Grid.SetItemText(m_CurrentRow, 4, str);
				pGridCellCheck = (CGridCellCheck*)m_Grid.GetCell(m_CurrentRow, 5);
				if (theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[j].LeadingZero) {
					pGridCellCheck->SetCheck(true);
				}
				str.Format(_T("%i"), theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[j].Grouping);
				m_Grid.SetItemText(m_CurrentRow, 6, str);
				str.Format(_T("%i"), theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[j].Digits);
				m_Grid.SetItemText(m_CurrentRow, 7, str);
				str.Format(_T("%.4f"), theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[j].Minimum);
		...
		m_OBLevelText.SetValue((cint)theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXLevel[theApp.m_CurPortIndex]);
		...
		m_Notes.SetWindowText(theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Notes);
...
void CDotMXTab::OnLampTest() 
{
	if (m_LampTest.GetCheck()) {
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_LampTest = 1;
	} else {
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_LampTest = 0;
	}
}

void CDotMXTab::OnChangeOBLevel() 
{
	if (theApp.m_CurrenthItem == NULL) return;

	theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXLevel[theApp.m_CurPortIndex] = (UCHAR)m_OBLevel.GetPos();
}

void CDotMXTab::OnChangeNotes()
{
	if (theApp.m_CurrenthItem == NULL) return;

	m_Notes.GetWindowText(theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Notes);
}

void CDotMXTab::OnClearButton() 
{
	if (theApp.m_CurrenthItem == NULL) return;

	theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].Clear();
...
void CDotMXTab::OnAdd() 
{
	AddItem(true);
}

void CDotMXTab::AddItem(BOOL defFlag) 
{
	int i;

	if (m_Grid.GetRowCount() >= DMX_NUM_ROWS) {
	...
	if (last_row != 0) {
		if (theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[last_row - 1].Grouping == 0 &&
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[last_row - 1].Digits == 0) {

			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[last_row - 1].VarTokenName.Empty();
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[last_row - 1].UserValue.Empty();
	...
	for (i = 0; i < last_row; i++) {
		grouping_cnt += (theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[i].Grouping + 
						theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[i].Digits);
		if (grouping_cnt >= 4) return;
	}
	...
	if (defFlag)
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].UserValue.Empty();
	...
	if (defFlag)
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].Factor = 1;
	...
	if (defFlag)
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].Constant = 0;
	...
	pGridCellNumeric->SetText("0");
	if (defFlag)
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].Minimum = 0;
...
void CDotMXTab::OnRemove() 
{
	if (m_Grid.IsValid(m_CurrentRow, 0) && m_CurrentRow != 0) {
		m_Grid.DeleteRow(m_CurrentRow);
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[m_CurrentRow - 1].VarTokenName.Empty();
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[m_CurrentRow - 1].VarTokenIndex = 0;
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[m_CurrentRow - 1].VarArrayTokenIndex = 0;
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[m_CurrentRow - 1].SimType = SIM_NONE;
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[m_CurrentRow - 1].UserValue.Empty();
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[m_CurrentRow - 1].Factor = 1;
		theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[m_CurrentRow - 1].Constant = 0;
		...
		NM_GRIDVIEW pItem;
		LRESULT pResult;
		for (pItem.iRow = 1; pItem.iRow < 5; pItem.iRow++) { // rows
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[pItem.iRow - 1].VarTokenName.Empty();
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[pItem.iRow - 1].VarTokenIndex = 0;
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[pItem.iRow - 1].VarArrayTokenIndex = 0;
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[pItem.iRow - 1].SimType = SIM_NONE;
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[pItem.iRow - 1].UserValue.Empty();
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[pItem.iRow - 1].Factor = 1;
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[pItem.iRow - 1].Constant = 0;
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[pItem.iRow - 1].LeadingZero = 0;
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[pItem.iRow - 1].Grouping = 0;
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[pItem.iRow - 1].Digits = 0;
			...
			for (pItem.iColumn = 1; pItem.iColumn < DMX_NUM_COLS; pItem.iColumn++) {	// cols
				...
				if (pItem.iColumn == 1) {	// VARIABLE
					theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[pItem.iRow - 1].VarTokenName = m_Grid.GetItemText (pItem.iRow, pItem.iColumn);
					theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[pItem.iRow - 1].VarTokenIndex = m_Grid.GetItemData (pItem.iRow, pItem.iColumn);
					theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[pItem.iRow - 1].SimType = m_Grid.GetItemData (pItem.iRow, pItem.iColumn + 1);
...
void CDotMXTab::OnClearData() 
{
	...
	// MINIMUM
	nCol = 8;
	theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[m_CurrentRow - 1].Minimum = 0;
	m_Grid.SetItemText(m_CurrentRow, nCol, "0");
...
void CDotMXTab::OnGridClick(NMHDR *pNotifyStruct, LRESULT* pResult)
{
    ...
    *pResult = 0;
	...
	switch (nCol) {
		case 1:	// VARIABLE
			...
			m_VarListDlg.m_VarTokenName = theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].VarTokenName;
			m_VarListDlg.m_VarArrayIndex = theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].VarArrayTokenIndex;
			nResponse = m_VarListDlg.DoModal();

			if (nResponse == IDOK) {
				theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].VarTokenName = m_VarListDlg.m_VarTokenName;
				theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].VarTokenIndex = m_VarListDlg.m_ArrayIndex;
				theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].VarArrayTokenIndex = m_VarListDlg.m_VarArrayIndex;
				theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].SimType = m_VarListDlg.m_SimType;
		...
		case 5:	// LEADING ZERO
			...
			if (pGridCellCheck->GetCheck()) {
				pGridCellCheck->SetCheck(false);
				theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].LeadingZero = 0;
			} else {
				pGridCellCheck->SetCheck(true);
...
void CDotMXTab::OnGridEndEdit(NMHDR *pNotifyStruct, LRESULT* pResult)
{
	...
	switch (nCol) {
		...
		case 2:	// USER VALUE
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].UserValue = m_Grid.GetItemText(nRow, nCol);
			break;

		case 3:	// FACTOR
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].Factor = atof(m_Grid.GetItemText(nRow, nCol));
			break;

		case 4:	// CONSTANT
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].Constant = atof(m_Grid.GetItemText(nRow, nCol));
			break;
		...
		case 6:	// GROUP ComboBox
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].Grouping = (UCHAR)atoi(m_Grid.GetItemText(nRow, nCol));
			break;

		case 7:	// DIGITS ComboBox
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].Digits = (UCHAR)atoi(m_Grid.GetItemText(nRow, nCol));
			break;

		case 8:	// MINIMUM
			theApp.m_pDevArray[theApp.m_CurDevIndex]->m_DotMXArray[theApp.m_CurItemIndex].m_Displays[nRow - 1].Minimum = atof(m_Grid.GetItemText(nRow, nCol));
			break;
...
BOOL CDotMXTab::OnCommand(WPARAM wParam, LPARAM lParam) 
{
