...
{	
	...
	m_strStandardFunction.Add("SIN"); //muﬂ in der Reihenfolge nach SINH stehen!!!
...
//	TRACE("Formel vorher: %s\n", strFormula);
	...
	strFormula.Replace(_T("(lmod(x,y))"),_T("lmod(x,y)"));
	strFormula.Replace(_T("(mod(x,y))"),_T("mod(x,y)"));

	// Einschlieﬂende Klammern beseitigen
	int i;
	for (i = 0; i < strFormula.GetLength(); i++)
	{
		switch (strFormula[i])
		{
			case '(': Level++; continue;
...
double CFormulaParser::SimpleExpression(int& nPosition, CString& strCharacter)
{
  ...
  while (strCharacter == "*" || strCharacter == "/" || strCharacter == "%")
    ...
    if (strOperator == "*")
		...
		s = (long)s % (long)Term(nPosition, strCharacter);
	}
	return s;
}
		...
		else
		{
			...
			for (wI = 1; wI <= AnzStdFunctions; wI++)
			{
				...
				if (strFunktionUpper == strDummy)
				{
					...
					else if (strFunktionUpper == "MOD") 
					{
						;
					}
					else if (strFunktionUpper == "LMOD") 
					{
						;
					}
...
double CFormulaParser::mod(int x, int y)    
{ 
	return x%y;
}

double CFormulaParser::lmod(long x, long y)    
{ 
	return x%y;
}