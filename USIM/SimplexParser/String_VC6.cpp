...
void CStringEx::PutDouble(double value, int len, char ExpChar)
{
	...
	if (Find('E') > 0 && ExpChar != 69)
		SetAt(Find('E') - 1, ExpChar);
	int acc = 16;
	if (len < acc + 1)
		return;
	Accuracy(acc, (int)ExpChar);
...
inline void CStringEx::dtoan(char *alpha, double val, int len)
{
	...
	{ // use E format 
		...
		if (val < 0.0)
			sprintf(format, "%c%d.%dE", '%', len - 1, len - 5);
      ...
      if (temp[i] == '0')
		...
		while (i>0 && temp[i] != 'E');
		if (temp[i] == 'E' && temp[i + 1] == '+')
			strcpy(&temp[i + 1], &temp[i + 2]);
		while (len < (int) strlen(temp) && i>3)
		{
      i--;
      strcpy(&temp[i], &temp[i + 1]);
		}
		if ((int)strlen(temp)>len)
			strcpy(alpha, "ERR");
	...
	{ // use f format 
		if (val < 0.0f)
			sprintf(format, "%c%d.%df", '%', len - 1, len - 3 - exp);
		else if (val < 0.)
			sprintf(format, "%c%d.%df", '%', len, len - 3);
		else if (val < 1.)
			sprintf(format, "%c%d.%df", '%', len, len - 2);
...
sprintf(format, "%c%d.%df", '%', len - 1, len - 3);
else if (val < 1.)
sprintf(format, "%c%d.%df", '%', len, len - 2);
else          sprintf(format, "%c%d.%df", '%', len, len - 2 - exp);
sprintf(alpha, format, val);
...
void CStringEx::TestExp()
{
	...
	if (GetAt(pos - 1) == 32)
		return;
	if (GetAt(pos - 1) == 69 || GetAt(pos - 1) == 101)
		return;
	Insert(pos - 1, 'E');
}
